# Category Performance Findings

## Analysis Period

2017-09-01 <= order_purchase_timestamp < 2018-09-01

Included orders:

delivered

## Overall Category Performance

### Top Categories by Item Sales

1. health_beauty
2. watches_gifts
3. bed_bath_table
4. sports_leisure
5. computers_accessories

### Sales Concentration

- Top 1 category share: 0.0968
- Top 5 category share: 0.4050
- Top 10 category share: 0.6236

## November 2017 Growth

Total Item Sales Change: 

+339,517.72

Top positive category changes:

1. bed_bath_table
2. health_beauty
3. furniture_decor
4. watches_gifts
5. toys

Key interpretation:

- 2017년 11월 Item Sales는 전월 대비 52.4% 증가했다. 이 시기 Buyer Count도 62.6% 증가했으며, 카테고리 기준으로는 bed_bath_table과 health_beauty의 판매금액 증가폭이 가장 컸다. 따라서 11월 성장은 특정 카테고리 한 곳에만 집중됐다기보다 구매고객 확대와 여러 주요 카테고리의 판매 증가가 함께 나타난 시기로 해석할 수 있다.

## December 2017 Decline

Total Item Sales Change:

-261,732.18

Largest negative category changes:

1. bed_bath_table
2. computers_accessories
3. furniture_decor
4. watches_gifts
5. cool_stuff

Key interpretation:

- 2017년 12월 Item Sales는 전월 대비 -26.5% 감소했다. 이 시기 Buyer Count도 -24.1% 감소했으며, 카테고리 기준으로는 bed_bath_table과 computers_accessories의 판매금액 감소폭이 가장 컸다. 따라서 12월 축소는 특정 카테고리 한 곳에만 집중됐다기보다 구매고객 확대와 여러 주요 카테고리의 판매 감소가 함께 나타난 시기로 해석할 수 있다.

## Limitations

- Item Sales is not accounting Net Sales.
- Product cost and margin are unavailable.
- Discount and refund amounts are unavailable.
- A single order or customer may appear in multiple product categories.
- Category-level Order Count and Buyer Count must not be summed to reproduce overall totals.
- Observed category changes do not establish causal effects.